...
exports.postOrder = async(req,res) => {
    ...
    const { productname, price, quantity } = req.body
        ...
        try {
            ...
            res.status(200).send({success : [{msg:"Commande envoyée...veuillez recevoir un mail de confirmation"}] , newOrder })

        } catch (error) {
            res.status(400).send({ errors : [{ msg : "Echec, Commande non envoyée ... "}]})   
        }
    }
